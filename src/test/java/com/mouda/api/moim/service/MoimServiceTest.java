...
class MoimServiceTest {
	...
	void 모임을_생성한다() {
		...
		Moim moim = MoimFixture.create(member, "testMoim");
	...
	void 현재_시점보다_이른_모임을_생성할_수_없다() {
		// given
		Member member = memberFixture.create("testMember");
		Moim moim = MoimFixture.create(member, "testMoim", LocalDate.now().minusDays(2));
	...
	void 이미_존재하는_모임_이름으로_생성할_수_없다() {
		// given
		Member member = memberFixture.create("testMember");
		Moim existedMoim = MoimFixture.create(member, "testMoim");
		moimService.createMoim(existedMoim, member);

		Moim newMoim = MoimFixture.create(member, "testMoim");
